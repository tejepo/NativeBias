...
###Race X Measures of Dehumanization: Whites
...
dfdehumanrace.white <- summarySE(data=dfdehumanDV.race, measurevar="White_Dehumanize", groupvars=c("Race"), na.rm = T)

pander(dfdehumanrace.white)

ggplot(dfdehumanrace.white, aes(x=Race, y=White_Dehumanize, fill=Race)) +
  geom_bar(position=position_dodge(), stat="identity",
           colour="black", # Use black outlines,
           size=.3) +      # Thinner lines
  geom_errorbar(aes(ymin=White_Dehumanize-ci, ymax=White_Dehumanize+ci),
                size=.3,    # Thinner lines
                width=.2,
                position=position_dodge(.9)) +
  xlab("Participant Race") +
  ylab("Measure of White Dehumanization (0-100)") +
  scale_y_continuous(limits = c(0,100)) +
  ggtitle("Do Different Racial Groups Support\n Have Similar Attitudes Toward Whites?") +
  theme_bw()

aov1 <- aov(White_Dehumanize ~ Race, dfdehumanDV.race)
summary(aov1)
TukeyHSD(aov1)
```
...
###Race X Measures of Dehumanization: Natives
Now let's look at those same tables and bar graphs for **Natives**:

```{r racedhumann,include=T,echo=F}
dfdehumanrace.native <- summarySE(data=dfdehumanDV.race, measurevar="Native_Dehumanize", groupvars=c("Race"), na.rm = T)

pander(dfdehumanrace.native)

ggplot(dfdehumanrace.native, aes(x=Race, y=Native_Dehumanize, fill=Race)) +
  geom_bar(position=position_dodge(), stat="identity",
           colour="black", # Use black outlines,
           size=.3) +      # Thinner lines
  geom_errorbar(aes(ymin=Native_Dehumanize-ci, ymax=Native_Dehumanize+ci),
                size=.3,    # Thinner lines
                width=.2,
                position=position_dodge(.9)) +
  xlab("Participant Race") +
  ylab("Measure of Native Dehumanization (0-100)") +
  scale_y_continuous(limits = c(0,100)) +
  ggtitle("Do Different Racial Groups Support\n Have Similar Attitudes Toward Natives?") +
  theme_bw()

aov2 <- aov(Native_Dehumanize ~ Race, dfdehumanDV.race)
summary(aov2)
TukeyHSD(aov2)
```
...
###Dehumanization and Holidays X Race: Dehumanizing Whites
...
```{r holiday1dvracewhite, include = T, echo = F}
ggplot(dfdehumanDV.race, aes(x= White_Dehumanize, y = No_Columbus)) +
  geom_point(position = position_jitter(width = 0.25)) +
  geom_smooth(stat = "smooth", position = "identity", method = "lm", se = T, na.rm = T) +
  xlab("Dehumanization (0-100)") +
  ylab("Support for Abolishing Columbus Day") +
...
```{r holiday2dvracewhite, include = T, echo = F}
ggplot(dfdehumanDV.race, aes(x= White_Dehumanize, y = Create_IPD)) +
  geom_point(position = position_jitter(width = 0.25)) +
  geom_smooth(stat = "smooth", position = "identity", method = "lm", se = T, na.rm = T) +
  xlab("Dehumanization (0-100)") +
...
###Dehumanization and Holidays X Race: Dehumanizing Natives
Now, onto the same analyses looking at Natives.

First **Columbus Day**:
```{r holiday1dvracenative, include = T, echo = F}
ggplot(dfdehumanDV.race, aes(x= Native_Dehumanize, y = No_Columbus)) +
  geom_point(position = position_jitter(width = 0.25)) +
  geom_smooth(stat = "smooth", position = "identity", method = "lm", se = T, na.rm = T) +
  xlab("Dehumanization (0-100)") +
  ylab("Support for Abolishing Columbus Day") +
...
Then **Indigenous Peoples' Day**:
```{r holiday2dvracenative, include = T, echo = F}
ggplot(dfdehumanDV.race, aes(x= Native_Dehumanize, y = Create_IPD)) +
  geom_point(position = position_jitter(width = 0.25)) +
  geom_smooth(stat = "smooth", position = "identity", method = "lm", se = T, na.rm = T) +
  xlab("Dehumanization (0-100)") +
  ylab("Support for Creating Indigenous Peoples' Day") +
