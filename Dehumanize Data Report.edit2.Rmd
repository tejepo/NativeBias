...
plot(dfdehumanDV$DH2_N_Civilized, dfdehumanDV$DH5_N_Childlike)
...
```{r}
lm11 <- lm(scale(No_Columbus,scale=F) ~ scale(DH5_N_Childlike,scale=F) + scale(DH5_W_Childlike,scale=F) + scale(DH5_N_Childlike,scale=F)*scale(DH5_W_Childlike,scale=F), dv4cor)
summary(lm11)
```
...
dv4cor2 <- dfdehumanDV %>%
  dplyr::select(SubjectNumber, No_Columbus, DH2_W_Civilized, DH2_N_Civilized)
...
```{r}
lm12 <- lm(No_Columbus ~ DH2_N_Civilized + DH2_W_Civilized + DH2_N_Civilized*DH2_W_Civilized, dv4cor2)
summary(lm12)
```

...and here it is centered:
```{r}
lm13 <- lm(scale(No_Columbus,scale=F) ~ scale(DH2_N_Civilized,scale=F) + scale(DH2_W_Civilized,scale=F) + scale(DH2_N_Civilized,scale=F)*scale(DH2_W_Civilized,scale=F), dv4cor2)
summary(lm13)
```
...
...and here it is centered:
```{r}
lm15 <- lm(scale(Create_IPD,scale=F) ~ scale(DH5_N_Childlike,scale=F) + scale(DH5_W_Childlike,scale=F) + scale(DH5_N_Childlike,scale=F)*scale(DH5_W_Childlike,scale=F), dv5cor)
summary(lm15)
```
...
dv5cor2 <- dfdehumanDV %>%
  dplyr::select(SubjectNumber, Create_IPD, DH2_W_Civilized, DH2_N_Civilized)
...
Here is the Civilized model uncentered:
```{r}
lm16 <- lm(Create_IPD ~ DH2_N_Civilized + DH2_W_Civilized + DH2_N_Civilized*DH2_W_Civilized, dv5cor2)
summary(lm16)
```

...and here it is centered:
```{r}
lm17 <- lm(scale(Create_IPD,scale=F) ~ scale(DH2_N_Civilized,scale=F) + scale(DH2_W_Civilized,scale=F) + scale(DH2_N_Civilized,scale=F)*scale(DH2_W_Civilized,scale=F), dv5cor2)
summary(lm17)
```
