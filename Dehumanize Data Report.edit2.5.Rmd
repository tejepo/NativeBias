...
plot(dfdehumanDV$N_Uncivilized, dfdehumanDV$DH5_N_Childlike)

dehum.nw.cor <- cor.test(dfdehumanDV$N_Uncivilized, dfdehumanDV$DH5_N_Childlike, method = ("pearson"))
...
dv4cor2 <- dfdehumanDV %>%
  dplyr::select(SubjectNumber, No_Columbus, DH2_W_Civilized, N_Uncivilized)
...
```{r}
lm12 <- lm(No_Columbus ~ N_Uncivilized + DH2_W_Civilized + N_Uncivilized*DH2_W_Civilized, dv4cor2)
summary(lm12)
```

...and here it is centered:
```{r}
lm13 <- lm(No_Columbus ~ scale(N_Uncivilized,scale=F) + scale(DH2_W_Civilized,scale=F) + scale(N_Uncivilized,scale=F)*scale(DH2_W_Civilized,scale=F), dv4cor2)
summary(lm13)
```
...
dv5cor2 <- dfdehumanDV %>%
  dplyr::select(SubjectNumber, Create_IPD, DH2_W_Civilized, N_Uncivilized)
...
Here is the Civilized model uncentered:
```{r}
lm16 <- lm(Create_IPD ~ N_Uncivilized + DH2_W_Civilized + N_Uncivilized*DH2_W_Civilized, dv5cor2)
summary(lm16)
```

...and here it is centered:
```{r}
lm17 <- lm(Create_IPD ~ scale(N_Uncivilized,scale=F) + scale(DH2_W_Civilized,scale=F) + scale(N_Uncivilized,scale=F)*scale(DH2_W_Civilized,scale=F), dv5cor2)
summary(lm17)
```
...
dv4and5cor2 <- dfdehumanDV %>%
  dplyr::select(SubjectNumber, Create_IPD, No_Columbus, N_Uncivilized)

dv4and5cor2.lng <- melt(dv4and5cor2, id=c("SubjectNumber", "N_Uncivilized"), variable.name = "Holiday_Support", value.name="Rating")

dv4and5cor2.lng.cen <- dv4and5cor2.lng
dv4and5cor2.lng.cen$N_Uncivilized <- scale(dv4and5cor2.lng.cen$N_Uncivilized, scale = F)
#dv4and5cor2.lng.cen$Rating <- scale(dv4and5cor2.lng.cen$Rating, scale = F)

lm3 <- lm(Rating ~ N_Uncivilized+ Holiday_Support + N_Uncivilized:Holiday_Support, dv4and5cor2.lng.cen)
summary(lm3)
```
...
dfdehumanrace.civilized <- summarySE(data=dfdehumanDV.race, measurevar="N_Uncivilized", groupvars=c("Race"), na.rm = T)

pander(dfdehumanrace.civilized)

ggplot(dfdehumanrace.civilized, aes(x=Race, y=N_Uncivilized, fill=Race)) +
  geom_bar(position=position_dodge(), stat="identity",
           colour="black", # Use black outlines,
           size=.3) +      # Thinner lines
  geom_errorbar(aes(ymin=N_Uncivilized-ci, ymax=N_Uncivilized+ci),
                size=.3,    # Thinner lines
                width=.2,
                position=position_dodge(.9)) +
  xlab("Participant Race") +
...
aov1 <- aov(N_Uncivilized ~ Race, dfdehumanDV.race)
summary(aov1)
TukeyHSD(aov1)
```
...
ggplot(dfdehumanDV.race, aes(x= N_Uncivilized, y = No_Columbus)) +
  geom_point(position = position_jitter(width = 0.25)) +
  geom_smooth(stat = "smooth", position = "identity", method = "lm", se = T, na.rm = T) +
...
ggplot(dfdehumanDV.race, aes(x= N_Uncivilized, y = Create_IPD)) +
  geom_point(position = position_jitter(width = 0.25)) +
  geom_smooth(stat = "smooth", position = "identity", method = "lm", se = T, na.rm = T) +
  xlab("Percptions of Natives: Civilized (0-100)") +
...
dvstereotypes <- dfdehumanDV.stereotypes %>%
  dplyr::select(SubjectNumber, No_Columbus, White_Dehumanize, Native_Dehumanize, DH2_W_Civilized, DH5_W_Childlike, N_Uncivilized, DH5_N_Childlike)
...
dvstereotypes.civil.c <- dfdehumanDV.stereotypes %>%
  dplyr::select(SubjectNumber, No_Columbus, White_Dehumanize, Native_Dehumanize, DH2_W_Civilized, N_Uncivilized)

dvstereotypes.civil.i <- dfdehumanDV.stereotypes %>%
  dplyr::select(SubjectNumber, Create_IPD, White_Dehumanize, Native_Dehumanize, DH2_W_Civilized, N_Uncivilized)
...
lm7 <- lm(scale(No_Columbus, scale=F) ~ White_Dehumanize + Native_Dehumanize + DH2_W_Civilized + N_Uncivilized, dvstereotypes.civil.c)
summary(lm7)
```
...
lm8 <- lm(scale(Create_IPD, scale=F) ~ White_Dehumanize + Native_Dehumanize + DH2_W_Civilized + N_Uncivilized, dvstereotypes.civil.i)
summary(lm8)
```
...
dvstereotypes.race <- dfdehumanDV.stereotypes %>%
  dplyr::select(SubjectNumber, White_Dehumanize, Native_Dehumanize, DH2_W_Civilized, N_Uncivilized, DH5_W_Childlike, DH5_N_Childlike, Race)
...
dvstereotypes.edu <- dfdehumanDV.stereotypes %>%
  dplyr::select(SubjectNumber, White_Dehumanize, Native_Dehumanize, DH2_W_Civilized, N_Uncivilized, DH5_W_Childlike, DH5_N_Childlike, College)
...
dvstereotypes.edu.q <- dfdehumanDV.stereotypes %>%
  dplyr::select(White_Dehumanize, Native_Dehumanize, DH2_W_Civilized, N_Uncivilized, DH5_W_Childlike, DH5_N_Childlike, College)
