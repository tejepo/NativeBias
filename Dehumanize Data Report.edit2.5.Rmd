...
ggplot(dfdehumanDV, aes(x=N_Uncivilized, y = DH5_N_Childlike)) +
  geom_point(position = position_jitter(width = 0.25)) +
  geom_smooth(stat = "smooth", position = "identity", method = "lm", se = T, na.rm = T) +
  xlab("Uncivilized (Natives)") +
...
dv4cor2 <- dfdehumanDV %>%
  dplyr::select(SubjectNumber, No_Columbus, N_Uncivilized)
...
```{r}
lm12 <- lm(No_Columbus ~ N_Uncivilized, dv4cor2)
summary(lm12)
```

...and here it is centered:
```{r}
lm13 <- lm(No_Columbus ~ scale(N_Uncivilized,scale=F), dv4cor2)
summary(lm13)
```
...
dv5cor2 <- dfdehumanDV %>%
  dplyr::select(SubjectNumber, Create_IPD, N_Uncivilized)
...
Here is the Civilized model uncentered:
```{r}
lm16 <- lm(Create_IPD ~ N_Uncivilized, dv5cor2)
summary(lm16)
```

...and here it is centered:
```{r}
lm17 <- lm(Create_IPD ~ scale(N_Uncivilized,scale=F), dv5cor2)
summary(lm17)
```
...
ggplot(dfdehumanrace.civilized, aes(x=Race, y=N_Uncivilized, fill=Race)) +
  ...
  geom_errorbar(aes(ymin=N_Uncivilized-ci, ymax=N_Uncivilized+ci),
                ...
                position=position_dodge(.9)) +
  xlab("Participant Race") +
  ylab("Measure of Attitudes Toward Natives: Civilized (0-7)") +
  scale_y_continuous(limits = c(0,7)) +
...
ggplot(dfdehumanDV.race, aes(x= N_Uncivilized, y = No_Columbus)) +
  geom_point(position = position_jitter(width = 0.25)) +
  geom_smooth(stat = "smooth", position = "identity", method = "lm", se = T, na.rm = T) +
  xlab("Percptions of Natives: Civilized (0-7)") +
  ylab("Support for Abolishing Columbus Day") +
  #labs(color='Race of\nParticipant') +
  facet_wrap(~ Race) +
  scale_y_continuous(limits = c(0, NA))
