...
colnames(dfdehumanDV) <- c(V1 = "DH1_W_Responsible",V2 ="DH2_W_Civilized",V3 = "DH3_W_Moral",V4 = "DH4_W_Polite",
                           ...
                           V60 = "Race",V61 = "StateNumeric",V62 = "LongestRegion",V63 = "SubjectNumber",V64 = "EMP_comp",V65 = "IMP_comp",V66 = "N_Uncivilized_E")
...
ggplot(dfdehumanDV, aes(x=N_Uncivilized_E, y = DH5_N_Childlike)) +
  geom_point(position = position_jitter(width = 0.25)) +
  geom_smooth(stat = "smooth", position = "identity", method = "lm", se = T, na.rm = T) +
  xlab("Other Endorsement Uncivilized (Natives)") +
  ylab("Childlike (Natives)") +
  scale_y_continuous(limits = c(0, NA))

dehum.nw.cor <- cor.test(dfdehumanDV$N_Uncivilized_E, dfdehumanDV$DH5_N_Childlike, method = ("pearson"))
...
dv4cor2 <- dfdehumanDV %>%
  dplyr::select(SubjectNumber, No_Columbus, N_Uncivilized_E)
...
ggplot(dv4cor2.lng, aes(x= Rating, y = No_Columbus, color = Group, group = Group)) +
  geom_point(position = position_jitter(width = 0.25)) +
  geom_smooth(stat = "smooth", position = "identity", method = "lm", se = T, na.rm = T) +
  xlab("Other Endorsement Uncivilized") +
  ylab("Support for No Columbus Day") +
  scale_y_continuous(limits = c(0, NA))
...
```{r}
lm12 <- lm(No_Columbus ~ N_Uncivilized_E, dv4cor2)
summary(lm12)
```

...and here it is centered:
```{r}
lm13 <- lm(No_Columbus ~ scale(N_Uncivilized_E,scale=F), dv4cor2)
summary(lm13)
```
...
dv5cor2 <- dfdehumanDV %>%
  dplyr::select(SubjectNumber, Create_IPD, N_Uncivilized_E)
...
Here is the Civilized model uncentered:
```{r}
lm16 <- lm(Create_IPD ~ N_Uncivilized_E, dv5cor2)
summary(lm16)
```

...and here it is centered:
```{r}
lm17 <- lm(Create_IPD ~ scale(N_Uncivilized_E,scale=F), dv5cor2)
summary(lm17)
```
...
dv4and5cor2 <- dfdehumanDV %>%
  dplyr::select(SubjectNumber, Create_IPD, No_Columbus, N_Uncivilized_E)

dv4and5cor2.lng <- melt(dv4and5cor2, id=c("SubjectNumber", "N_Uncivilized_E"), variable.name = "Holiday_Support", value.name="Rating")

dv4and5cor2.lng.cen <- dv4and5cor2.lng
dv4and5cor2.lng.cen$N_Uncivilized_E <- scale(dv4and5cor2.lng.cen$N_Uncivilized_E, scale = F)
#dv4and5cor2.lng.cen$Rating <- scale(dv4and5cor2.lng.cen$Rating, scale = F)

lm3 <- lm(Rating ~ N_Uncivilized_E+ Holiday_Support + N_Uncivilized_E:Holiday_Support, dv4and5cor2.lng.cen)
summary(lm3)
```
...
dfdehumanrace.civilized <- summarySE(data=dfdehumanDV.race, measurevar="N_Uncivilized_E", groupvars=c("Race"), na.rm = T)

pander(dfdehumanrace.civilized)

ggplot(dfdehumanrace.civilized, aes(x=Race, y=N_Uncivilized_E, fill=Race)) +
  geom_bar(position=position_dodge(), stat="identity",
           colour="black", # Use black outlines,
           size=.3) +      # Thinner lines
  geom_errorbar(aes(ymin=N_Uncivilized_E-ci, ymax=N_Uncivilized_E+ci),
                size=.3,    # Thinner lines
                width=.2,
                position=position_dodge(.9)) +
  xlab("Participant Race") +
  ylab("Measure of Attitudes Toward Natives: Other Endorsement Uncivilized (0-7)") +
  scale_y_continuous(limits = c(0,7)) +
  ggtitle("Do Different Racial Groups\n Have Similar Attitudes Toward Natives?") +
  theme_bw()

aov1 <- aov(N_Uncivilized_E ~ Race, dfdehumanDV.race)
summary(aov1)
TukeyHSD(aov1)
```
...
ggplot(dfdehumanDV.race, aes(x= N_Uncivilized_E, y = No_Columbus)) +
  geom_point(position = position_jitter(width = 0.25)) +
  geom_smooth(stat = "smooth", position = "identity", method = "lm", se = T, na.rm = T) +
  xlab("Percptions of Natives: Other Endorsement Uncivilized (0-7)") +
  ylab("Support for Abolishing Columbus Day") +
  #labs(color='Race of\nParticipant') +
  facet_wrap(~ Race) +
  scale_y_continuous(limits = c(0, NA))
...
ggplot(dfdehumanDV.race, aes(x= N_Uncivilized_E, y = Create_IPD)) +
  geom_point(position = position_jitter(width = 0.25)) +
  geom_smooth(stat = "smooth", position = "identity", method = "lm", se = T, na.rm = T) +
  xlab("Percptions of Natives: Other Endorsement Uncivilized (0-7)") +
...
dvstereotypes <- dfdehumanDV.stereotypes %>%
  dplyr::select(SubjectNumber, No_Columbus, White_Dehumanize, Native_Dehumanize, DH2_W_Civilized, DH5_W_Childlike, N_Uncivilized_E, DH5_N_Childlike)
...
dvstereotypes.civil.c <- dfdehumanDV.stereotypes %>%
  dplyr::select(SubjectNumber, No_Columbus, White_Dehumanize, Native_Dehumanize, DH2_W_Civilized, N_Uncivilized_E)

dvstereotypes.civil.i <- dfdehumanDV.stereotypes %>%
  dplyr::select(SubjectNumber, Create_IPD, White_Dehumanize, Native_Dehumanize, DH2_W_Civilized, N_Uncivilized_E)
...
lm7 <- lm(scale(No_Columbus, scale=F) ~ White_Dehumanize + Native_Dehumanize + DH2_W_Civilized + N_Uncivilized_E, dvstereotypes.civil.c)
summary(lm7)
```
...
lm8 <- lm(scale(Create_IPD, scale=F) ~ White_Dehumanize + Native_Dehumanize + DH2_W_Civilized + N_Uncivilized_E, dvstereotypes.civil.i)
summary(lm8)
```
...
dvstereotypes.race <- dfdehumanDV.stereotypes %>%
  dplyr::select(SubjectNumber, White_Dehumanize, Native_Dehumanize, DH2_W_Civilized, N_Uncivilized_E, DH5_W_Childlike, DH5_N_Childlike, Race)
...
dvstereotypes.edu <- dfdehumanDV.stereotypes %>%
  dplyr::select(SubjectNumber, White_Dehumanize, Native_Dehumanize, DH2_W_Civilized, N_Uncivilized_E, DH5_W_Childlike, DH5_N_Childlike, College)
...
dvstereotypes.edu.q <- dfdehumanDV.stereotypes %>%
  dplyr::select(White_Dehumanize, Native_Dehumanize, DH2_W_Civilized, N_Uncivilized_E, DH5_W_Childlike, DH5_N_Childlike, College)
